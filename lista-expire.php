...
<?php
		if ($conn) {
			...
			$search_result = ldap_list($ldap_con, "cn=users,dc=wipsad,dc=local", "(&(uid=*)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(accountexpires=9223372036854775807))(!(accountexpires=0)))");
				...
				<form action="index.php?podstrona=usun" method="POST">
					...
					<table id="usrTable" class="tableT" cellpadding="2">
						<tr>
							...
							<th>Nr UID &nbsp;</th>
							<th>Katalog domowy WINDOWS &nbsp;</th>
							<th>Katalog domowy UNIX &nbsp;</th>
							<th>Grupa &nbsp;</th>
							<th id="defaultSort_th">Data wygaśnięcia &nbsp;</th>
...
<?php
						for ($entry_num = 0; $entry_num < /*100*/ $result_entries['count']; $entry_num++) {
							...
							if ($lastlogon > date("Y-m-d", strtotime("-14 days"))) {
											$lastlogon = "<i><font color=gray>".$lastlogon."</font></i>";
							}
...
?>						
							<tr>
								...
								<td align=right><?php echo @$result_entries[$entry_num]['uidnumber'][0]; ?></td>
